...
public class BuyItemConfirmAction extends ActionSupport implements SessionAware{
	...
	public String execute() throws SQLException{
		...
		for(int i =0; i<buyItemDTOList.size(); i++){
			int s = buyItemDTOList.get(i).getItem_stock();

			int id = buyItemDTOList.get(i).getId();

			int p = buyItemDTOList.get(i).getTotal_price();
			String total_price = String.valueOf(p);

			int count = buyItemDTOList.get(i).getCount();

			int item_stock =s - count;
